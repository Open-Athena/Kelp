        ...
        for entry in entries:
            if node_type in STATEMENT_TYPES:
                try:
                    ...
                    pytest.fail(f"Statement entry failed to parse: type={node_type}, source={entry.source!r}")
            else:
                try:
                    ast.parse(f"__x = {entry.source}")
                except SyntaxError:
                    pytest.fail(f"Expression entry failed to parse: type={node_type}, source={entry.source!r}")
...
def test_deduplication(bank):
    ...
    for node_type, entries in bank.entries.items():
        ...
        assert len(sources) == len(set(sources)), (
            f"Duplicates found in {node_type}: {len(sources)} entries but {len(set(sources))} unique"
        )
